    ...
    function apply(ctx) {
      ctx.slots.inject("conversation.input.dock", function () {
        return ctx.slots.register(
          { name: "conversation.input.dock", id: "balance-panel", order: 100, label: "Balance Panel" },
          react.memo(function BalanceDock(props) {
            var pressure = props.useProjection ? props.useProjection("contextPressure") : undefined;
            // Queue strip (a sibling dock entry) has margin-bottom: -(gap+3px) to
            // tuck against the composer card; compensate while it is visible.
            var queueRows = props.useSession ? props.useSession(function (s) { return s.queue || []; }) : [];
            var queueVisible = queueRows.some(function (r) { return r && r.placement === "queued"; });
            var rowStyle = queueVisible ? { marginTop: "calc(var(--dsh-composer-stack-gap) + 3px)" } : undefined;
