...
const STATE_PATH = '/dsh-balance-hud/state'
...
// Wait for the web server before apply: the state route must be registered
// after webServer is up, otherwise the browser half's polls fall through to
// the SPA fallback (index.html) and balance/spend never load.
...
export function apply(ctx) {
  ...
  async function fetchBalanceNow() {
    ...
    try {
      ...
      console.error('[dsh-balance-hud] credentials.resolve failed', err)
      return null
    }
    ...
    try {
      ...
      console.error('[dsh-balance-hud] balance fetch failed', err)
      return null
    }
  }
      ...
      for await (const chunk of next()) {
        if (chunk && chunk.type === 'usage' && chunk.usage) {
          try {
            ...
            console.error('[dsh-balance-hud] usage record failed', err)
          }
        }
        yield chunk
      }
  ...
  if (webServer !== undefined) {
    ...
    webServer.register({
      ...
      console.error('[dsh-balance-hud] state route registration failed', err)
    }
  }

  console.log('[dsh-balance-hud] host ready')
}
